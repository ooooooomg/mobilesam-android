 ...
 *
 * Input: RGB bitmap. Output: (1, 256, 64, 64) embedding as FloatArray.
 * Preprocessing matches the Python reference:
 * ResizeLongestSide(1024) -> normalize -> pad to 1024x1024.
 */
class SamImageEncoder(
    ...
    init {
        ...
        android.util.Log.d("MobileSAM", "encoder CPU threads=$cores")
    }
    ...
    fun preprocess(bitmap: Bitmap): Preprocessed {
        ...
        // Read pixels into a padded 1024x1024 ARGB buffer (zeros = black)
    ...
    /** Encode a bitmap to (1, 256, 64, 64) embedding. */
    fun encode(bitmap: Bitmap): Pair<FloatArray, Preprocessed> {
        ...
        tensor.close()
        // onnx output shape (1, 256, 64, 64), flattened. Result.get(int)
